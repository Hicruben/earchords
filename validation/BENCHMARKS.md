...
**已实测该路径的最可行种子(2026-07,第三条负结果)**:用现有 GuitarSet 训练一个
学习型线性打分器(逻辑回归,chroma→24 类 maj/min)替代手工模板,按 excerpt 分组
5 折交叉验证:**学习型 65.9% << 手工模板 72.6%**;迁移到合成集 **88.8% << 94.7%**。
原因:29 首/3499 帧太少太窄,学不出移调不变的和弦结构,而手工模板已编码最优移调
不变先验(12 根音同构)。**结论:简单学习型模型用现有数据反而更差;要超越手工
模板必须有大规模多样标注(数千首跨调跨曲风)+ 移调等变架构(CNN)。** 这把"训练
神经模型"这条路的前提(大标注数据)钉死了——需维护者提供数据集才能启动。
