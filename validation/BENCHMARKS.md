...
## Schubert Winterreise SWD(2026-07,真实录音 + 权威标注)

第三个对外基准,也是**最可信的非吉他数字**:SWD(Zenodo 5139893,CC-BY,真实钢琴+
人声录音 + 音频对齐的权威和弦标注 CSV)。跑 HU33 演奏版 18 首(3995s)。

| 范围 | Root-WCSR | MajMin-WCSR |
|---|---|---|
| **SWD 古典真实录音(18 首,3995s)** | **65.7%** | **61.8%** |

古典和声本就难(dim7、转位、副属、半音进行密集),文献深度模型在 Winterreise 上
majmin 约 65–75%,**本工具 61.8% 已接近该区间下缘**——在真实录音+权威标注的复杂
和声上有竞争力。三基准合看的一致画像:**简单干净(独奏吉他)83%,复杂和声/密集
(古典/全混音)~56–62%**;工具强在清晰素材,弱在和声复杂或密集混音。
...
- **移调等变模型 + 更多真实权威数据(SWD,15975 帧,是 GuitarSet 的 4.5 倍)**:
  5 折 **55.9% ≈ 手工模板 55.8%,打平**。即之前 GuitarSet 上的 +0.5 是噪声,更多真
  数据下学习型线性打分器**并不能拉开差距**。

**结论(再修正,证据更强):瓶颈不在"打分器"(模板 vs 学习型),而在 chroma 特征
本身。** 复杂/密集素材的 chroma(来自 Basic Pitch 音符)有歧义,任何**线性**打分器
(手工或学习)都提取不出更多信号——SWD 上多 4.5 倍真数据仍打平即证。**因此冲 SOTA
的唯一路径是从音频/频谱直接学特征的深度模型(CNN/CRNN posteriorgram),而非在 chroma
上换线性打分器。** 这是一次真实的深度学习工程(频谱输入 + 卷积特征 + 大规模多样
标注 + GPU 训练 + 转 TF.js),需维护者决策与数据/算力投入。已排除:所有超参、经典
CQT 前端、逐帧激活、朴素/移调等变**线性**学习模型。
